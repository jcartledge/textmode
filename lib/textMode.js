...
const numberInRange = Symbol('numberInRange');
...
class TextMode {
  ...
  set row (row) {
    this[setPos](this[numberInRange](row, this.numRows - 1), this.col);
  }
  ...
  set col (col) {
    this[setPos](this.row, this[numberInRange](col, this.numCols - 1));
  }
   ...
   *
   * If only one argument is passed it is treated as position, counted from top
   * left across then down.
   *
   * @param {number} rowOrPos - Row to move to, or absolute position if no col supplied.
   * @param {number} col - Column to move to.
   * @return {TextMode} - TextMode instance for chaining.
   */
  moveTo (rowOrPos, col = null) {
    if (col === null) {
      this.pos = this[numberInRange](rowOrPos, this.textBuffer.length - 1);
    } else {
      this.row = rowOrPos;
      this.col = col;
    }
